...
EDU01a_transformed_data
```

```{r}
EDU01b_transformed_data
```
...
```{r}
combined_enrollment <- function(urla, urlb, value = "count") {
  data_list_a <- my_wrapper(url = urla, value = value)
  data_list_b <- my_wrapper(url = urlb, value = value)
  county_df <- dplyr::bind_rows(data_list_a[[1]], data_list_b[[1]])
  state_df <- dplyr::bind_rows(data_list_a[[2]], data_list_b[[2]])
  combined_data_list <- list(county_df, state_df)
  return(combined_data_list)
}

census_EDU01 <- combined_enrollment(urla = "https://www4.stat.ncsu.edu/~online/datasets/EDU01a.csv",
...
```{r}
plot.county <- function(df, var_name = "count", state_selection = "NC",
                        top = TRUE, num_entries = 5) {
  y <- df |>
    filter(state == paste0(" ", state_selection)) |>
    group_by(area_name) |>
    summarise(Mean = mean(get(var_name), na.rm = TRUE))
  y_fil <- ifelse(top == TRUE, arrange(y, desc(Mean)), arrange(y, Mean))
# Having difficulty returning the 'num_entries'. Keep getting all area_name entries, as opposed to just the number specified.
    y_head <- head(y_fil, num_entries)
  y_head
}
