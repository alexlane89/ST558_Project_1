...
add_yr_meas <- function(x, value = "count") {
  x |>
    mutate(year = as.numeric(paste0("19", substr(enrollment,
                                            nchar(enrollment) - 2,
                                            nchar(enrollment) - 1))),
      measurement = substr(enrollment, 1, nchar(enrollment) - 3))|>
  select(area_name, STCOU, year, measurement, value)# |>
#  ifelse(year == 1900, mutate(year = 2000))
}
...
> Create a plotting function to show means of each enrollment count for years from the state data set.
