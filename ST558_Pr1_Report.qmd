...
> Repeat the steps above, but within the context of a function to generalize the actions.
...
add_yr_meas <- function(x, value = "count") {
  x |>
    mutate(
      year = as.numeric(paste0("19", substr(enrollment,
                                            nchar(enrollment) - 2,
                                            nchar(enrollment) - 1))),
      measurement = substr(enrollment, 1, nchar(enrollment) - 3)
      )|>
...
```{r}
sep_county_data <- function(x, value = "count") {
  fcn_indices <- grep(pattern = ", \\w\\w", x$area_name)
  fcn_county_data <- x[fcn_indices, ]
  class(fcn_data) <- c("county", class(fcn_data))
